...
const get = <T>(key: string, fallback: T, schema: ZodSchema<T>) =>
  defaultStore.get(key, fallback, schema);
...
const removeAsync = async (key: string, options?: SecureStoreOptions) =>
  await defaultStore.removeAsync(key, options);
